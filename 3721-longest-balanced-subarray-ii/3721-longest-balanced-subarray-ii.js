...
var longestBalanced = function(nums) {
    ...
    class SegTree {
        ...
        addRange(idx, l, r, ql, qr, val) {
            ...
            this.push(idx);
            const mid = (l + r) >> 1;
            if (ql <= mid) this.addRange(idx<<1, l, mid, ql, Math.min(qr, mid), val);
            if (qr > mid)  this.addRange(idx<<1|1, mid+1, r, Math.max(ql, mid+1), qr, val);
            this.pull(idx);
        }
        add(l, r, v) {
            if (l > r) return;
        ...
        findRightmostZero(idx, l, r, ql, qr) {
            ...
            const mid = (l + r) >> 1;
            if (qr > mid) {
                const res = this.findRightmostZero(idx<<1|1, mid+1, r, Math.max(ql, mid+1), qr);
                if (res !== -1) return res;
            }
            if (ql <= mid) {
